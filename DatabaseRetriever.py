    ...
    def fetch_contact_person_nr_and_username(self, first_name, last_name):
        ...
        for row in values:
            if row[FIRST_NAME_COLUMN] == first_name and row[LAST_NAME_COLUMN] == last_name:  # it first checks if the first element of the row is the name
                return "+" + row[CONTACT_COLUMN][2:] + " " + row[USERNAME_COLUMN] if len(
                    row) > 1 else None  # if the row is not empty, it returns the value in the 2nd cell.
        return None

    def fetch_info_on_startup(self, code):  # very similar to fetch_contact_nr, but it returns the value in the 3rd cell, if it is not empty.
        result = self.service.spreadsheets().values().get(
            spreadsheetId=self.login_spreadsheet_id,
            range=data_range
        ).execute()
        ...
        for row in values:
            if row[WATCH_CODE_COLUMN] == code:
                value = "f: " + row[FIRST_NAME_COLUMN] + " " if len(row) > 1 else None
                value += "l: " + row[LAST_NAME_COLUMN] + " " if len(row) > 1 else None
                value += "p: " + row[CONTACT_COLUMN] + " " if len(row) > 1 else None
                value += "u: " + row[USERNAME_COLUMN] + " |end|"if len(row) > 1 else None
...
# spreadsheetId = '1ECCL-Nr28LnLU-2ut3moZnOwGhVUlK7tOe4lue0S1K0' # example. the ID can be taken from the link to the sheet, ex: https://docs.google.com/spreadsheets/d/<ID>/edit#gid=0

# api = DatabaseRetriever(credentialsFile, spreadsheetId)

# nameToLookup = 'Sai'
# nameToLooku = 'Ba Madhaf'
# print(api.fetch_contact_person_nr(nameToLookup,nameToLooku))
